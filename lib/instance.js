...
function resolveDerivedPath(rootDir, value, fallback) {
  const rawValue = String(value || "").trim();
  if (!rawValue) {
...
function deriveInstancePaths(rootDir, instanceRootValue, overrides = {}) {
  const instanceRoot = resolveInstanceRoot(rootDir, instanceRootValue);
  const contentRoot = resolveDerivedPath(
    rootDir,
    overrides.contentRoot,
    path.join(instanceRoot, "content")
  );
