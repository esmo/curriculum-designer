...
function resolveInstanceRuntime(input) {
  ...
  const explicitRoot = String(input.instanceRoot || "").trim();
  const explicitName = String(input.instanceName || "").trim();

  if (explicitRoot) {
    return {
      instanceName: explicitName,
      registryFile: explicitName
        ? resolveRegistryFile(input.registryFile)
        : "",
      source: "root",
      paths: deriveInstancePaths(rootDir, explicitRoot),
      adminPort: parseAdminPort(input.adminPort, DEFAULT_ADMIN_PORT),
    };
  }
  ...
  return {
    ...
    registryFile: "",
    source: "default",
    paths: deriveInstancePaths(rootDir, ""),
    adminPort: parseAdminPort(input.adminPort, DEFAULT_ADMIN_PORT),
  };
}
